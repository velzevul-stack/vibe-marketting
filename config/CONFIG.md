...
3. **Назначить прокси аккаунтам** (главное меню **8** → **1**) — перераспределяет прокси из пула по аккаунтам в accounts.json
...
### Сбор базы: меню 2 → 1 (один аккаунт)

- **П.2 → 1 → общий** — выбор строки из `accounts.json`, для этого прогона сбор **без прокси** (ни пул, ни поле `proxy` у аккаунта).
- **П.2 → 1 → отдельный** — новая сессия: имя файла, api (можно из `telethon_default_api`), телефон, затем вопрос **с прокси или без**, код и при необходимости 2FA в консоли; после сбора можно записать аккаунт в `accounts.json`.
- **П.2 → 2** — прежний режим: настройки `scrape_use_proxy` / пул / закрепление `scrape_session_name` из `settings.json`.
