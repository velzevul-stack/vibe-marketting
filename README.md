...
## Меню (пункты **1–9** по порядку, затем **a**, **b**; **0** — выход)

| Ключ | Действие |
|------|----------|
| **1** | Поиск групп (RapidAPI, TGStat, Telemetr, TG Catalog, DuckDuckGo, groups.txt) |
| **2** | Просмотр найденных групп (`output/found_groups.json`) |
| **3** | Статистика базы |
...
| **9** | Хаб: импорт ZIP, настройки, сессии Telethon, API my.telegram (заглушка), подготовка аккаунтов |
| **a** | Очистить `found_groups.json` |
| **b** | Фильтр базы продавцов (удалить записи без признаков РБ) |
