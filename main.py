...
from src.broadcast.csv_cli import parse_csv_minutes_interval, run_csv_broadcast_cli
...
def _cli_broadcast(
    ...
    *,
    ...
    if broadcast_delay_spec is not None:
        try:
            sd_lo, sd_hi = parse_csv_minutes_interval(
...
def _cli_csv_broadcast(
    ...
    *,
    ...
    """Обёртка CLI → ``run_csv_broadcast_cli``."""
    return run_csv_broadcast_cli(
        dir_path,
        csv_path,
        delay_minutes_spec,
        zip_conflict,
        ...
        sent_log=sent_log,
        skip_sent=skip_sent,
        csv_account_gap_spec=csv_account_gap_spec,
    )
