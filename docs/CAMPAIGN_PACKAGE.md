...
Одна строка — один аккаунт. **Три или четыре поля**, разделённые пробелами:

```text
api_id:api_hash <прокси_одной_строкой> <stem> [<phone>]
```
...
87654321:fedcba0987654321fedcba098765 10.0.0.5:8000:login:password acc02 +375291234567
```

- Во втором поле **не должно быть пробелов** (URL или `host:port:user:pass`).
- Четвёртое поле — **телефон** в международном формате **без пробелов** (например `+375291234567`), опционально; попадает в `phone` в `accounts.json` при назначении из пакета.
...
1. **`sessions_bind.txt`** — полная привязка api + proxy + stem (+ опционально phone).
