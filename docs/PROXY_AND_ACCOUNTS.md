...
| Назначить прокси (меню **9 → 2 → 2**) | Перераспределяет пул по аккаунтам |
...
| Поле | Описание |
|------|----------|
| `session_name` | Уникальное имя файла без `.session` (из ZIP — как в архиве; при **новом входе по телефону** в меню по умолчанию — **все цифры номера**, например `375291234567`) |
...
- **Новая авторизация по телефону** — создаёт `.session` там же (по умолчанию имя = цифры номера)
...
2. Назначает прокси из пула всем аккаунтам (как **9 → 2 → 2**), **не затирая** прочие строки в `accounts.json`.
...
### Назначить прокси аккаунтам (меню 9 → 2 → 2)

Если прокси заданы в пуле (`proxies.txt` или `settings.json`): главное меню **9** (хаб) → **2** (настройки) → **2** (назначить прокси round-robin).
