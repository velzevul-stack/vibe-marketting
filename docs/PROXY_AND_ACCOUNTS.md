...
1. Если у аккаунта **ещё нет** облачного пароля 2FA — включает его (**без email**, пустой hint). Пароль: **`bulk_2fa_password`** в `settings.json` или ввод в консоли (скрытый). Не коммитьте пароль в git.
